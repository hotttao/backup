...
description: "通过示例认识 DolphinScheduler 与三节点架构"
featuredImage:
...
```text
1. Schedule 到期或用户手工触发后，API/Scheduler 在 Metadata DB 创建 Command
2. 某个 Master 领取 Command，创建 Workflow Instance，并计算第一批可运行 TaskInstance
3. Master 按 Task 配置选择 Worker Group
4. Master 根据 Registry 中的存活和负载信息选择具体 Worker，通过 RPC 主动下发任务
5. Worker ACK 后运行 Shell、Python、SQL、Spark 等 Task Plugin，并持续回报状态
6. Master 更新 TaskInstance，依据 DAG 依赖创建或提交下一批任务
7. 所有节点进入终态后，Master 更新 Workflow Instance，并产生告警或补数相关状态
```
