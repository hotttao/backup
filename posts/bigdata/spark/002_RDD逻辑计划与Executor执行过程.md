...
MEMORY_AND_DISK 中的 disk 通常是 Executor 本地存储。Persist 是可丢失的性能优化：

- Executor 退出或 block 被驱逐后可能丢失；
- 丢失后根据 lineage 重算；
- 不会自动截断 lineage。

### 8.2 Checkpoint 建立可靠恢复起点
...
Checkpoint 把 RDD records 写入可靠分布式存储。成功后可以从 checkpoint RDD 开始恢复，不必回溯完整旧 lineage。
...
~~~text
BlockManager 未找到 block
    → 根据 Dependency 找 parent
...
重算通常要求输入仍可读、用户函数仍存在且计算具有确定性。Spark 不能自动撤销外部副作用：

~~~python
rdd.foreach(lambda record: call_external_api(record))
~~~

Task 在调用部分 API 后失败并重试，可能重复调用。外部写入要使用幂等键、事务或两阶段提交。

Driver 完全故障时，普通 RDD lineage 主要保存在原 Driver 内存中，不能等同于完整应用恢复。Structured Streaming 的查询 checkpoint 还会记录 source offset、批次进度和状态存储元数据，和普通 RDD checkpoint 不同。
