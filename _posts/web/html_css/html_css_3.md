...
CSS 布局是学习 HTML&CSS 的第一个难点，我们分三节来搞定他。
<!-- more -->
...
本节我们主要来介绍前两种传统的布局方式 position 和 float。我们将在接下来的两节中分别介绍弹性盒子和多列布局，网格。
...
绝对定位元素的参考点取决于绝对定位元素的父元素的position属性:
...
"子绝父相"，即子元素使用绝对定位，父元素使用相对定位是我们 CSS 布局中常用的定位方式，这样可以帮助我们将子元素的定位限定在父元素内。
